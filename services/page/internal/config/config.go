...
// vừa consume domain event workspace từ workspace.events (dựng projection authz).
// Brokers: danh sách địa chỉ broker; UserEventsTopic: topic consume event user;
// WorkspaceEventsTopic: topic consume event workspace (page dựng projection membership);
...
func Load() (*Config, error) {
	...
	v.SetDefault("server.http_port", 8004)
	v.SetDefault("server.grpc_port", 9004)
	...
	v.SetDefault("kafka.consumer_group", "page-service")
	v.SetDefault("otel.enabled", false)
	v.SetDefault("otel.service_name", "page-service")
